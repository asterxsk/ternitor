---
name: Ternitor
description: A film cutting bench: blank frames on a perforated rail, marked in grease pencil.
colors:
  ground: "#000000"
  band: "#0C0C0C"
  speck: "#121212"
...
cannot have a surface that performs. So the surface is a **film cutting bench**: a
perforated rail crossing the window, frames hanging in the open bin below it, a
mechanical counter, and a grease pencil. It is short, legible in one look, and
each element is a real instrument rather than a decoration.

The figure is the app: **every frame on the rail is blank**, a film cell with a
title bar and nothing under it, because the window Ternitor hides is by definition
a window with nothing in it. That pun is the entire visual idea, and it is only
honest because the count driving it comes straight from the janitor.

Five rules hold the surface together:

1. **The rail advances; it does not browse.** Frames enter at the right and leave
   at the left, and the strip physically discards them. Nothing on the surface is
   clickable, hoverable or scrollable except the two controls. The app keeps no
   history, so the design must not imply one.
2. **Every frame is blank.** The figure is a picture of *blank* windows.
3. **Orange is grease pencil, and grease pencil is the operator's hand.** It
   appears in exactly two places: the cut mark on the newest frame, and the tick
   beside an armed switch. Never as a fill, never as a state colour.
4. **State is a mark, not a colour.** ON is a punched hole; OFF is an intact
   strip. The whole surface reads correctly in greyscale.
5. **Aperture, not container.** The rail runs off both edges. The film continues
   past the frame we happen to be looking at.

There is no light theme. A cutting bench is a dark room with a lit viewer, and
this surface is built around one black ground with ink lifted off it; inverting it
...
- `band` is the rail the strip runs over, one step above the base so the rail
  reads as a distinct surface without a line being drawn.
- `speck` and `dust` are grain: one deterministic scatter of single pixels across
  ground and rail alike. `dust` is the rare speck that catches light. Both are
  baked, never animated, so the bench does not crawl between repaints.
- `edge` is one-pixel structure: frame outlines, perforation outlines, counter
  dividers. It is deliberately *below* body contrast — structure should be felt
  before it is seen.
...
- `grease` is the one saturated value on the surface, and it is rationed. It means
  *the operator's hand was here*: the cut mark on the newest frame and the tick
  beside an armed switch. Nothing else may use it.
...
- **mono — Cascadia Mono.** Data: frame counts, window titles, clock times, the
  version string. The terminal face of the audience this app serves.
...
A fixed 720×428 logical client at 96dpi, scaled as a whole by the monitor's DPI
...
|---|---|---|
| Rail | 0–84 | Perforations at 8–15 and 77–83; 13 frame cells of 58px in the band at 20–72 |
| Head | 104–196 | Name and description left; counter right |
| Fields | 228–332 | Last cut, session clock left; the switch and its small print right |
| Footer | 368–404 | Version, target, log location |

The rail runs edge to edge with no end caps, anchored 6px from the right so the
newest frame is always fully visible. The thirteen cells are laid out from that
anchor leftward, so when all of them are lit the thirteenth is clipped by the
window edge — the oldest frame is visibly leaving the bench.
...
When every one of the thirteen cells is lit the rail is full: it stops advancing,
and the counter above carries the count from there. A saturated tally is honest —
it reads as a strip with no room left — where a strip that kept shuffling
identical blank frames would be pretending to move.
...
lines at two weights (`edge` for structure, `rule` for division) and by the single
step between `ground` and `band`.
...
Rectangles and one round. Frame cells, the counter housing, its wheels, the rules
and the rail are all rectangular with square corners: this is cut film and cut
metal.

The only rounded forms are the **sprocket holes** (4px) and the **punch control**
(4px, with a 2px slug inside), because those two are the shapes that are literally
*punched* rather than cut. That distinction is the shape system: cut is square,
punched is round.
...
**The rail.** A 84px band across the full width, with two rows of sprocket holes
and a divider between every pair of 58px cells. It is baked into the background
bitmap, not redrawn per frame.

**The frame.** One per hidden console, up to thirteen, filling from the right. A
lit frame carries an inner rectangle inset 7px with a title-bar line 8px below its
top and nothing beneath — a window with nothing in it. An unlit frame carries
nothing, so lit and unlit read apart without a fill colour.

**The cut mark.** On the newest frame only, and it moves: when a new console is
hidden the previous frame loses its mark and the new one takes it. This is what
keeps orange to exactly one mark on the surface, and it makes the accent mean
*recency* rather than *state*.
...
- Put each fact in the instrument that fits it: counts in wheels, states as marks,
  time as clock readings, windows as frames.
...
- Don't add a scrollbar, a hover tooltip, a per-frame click target, or anything
  else that turns the rail into a browsable history. The app keeps no history.
...
- Don't add a light theme without redrawing the object; inverting the palette
  produces a film strip on white paper, which is a different thing.
